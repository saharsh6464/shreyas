...
export const QuestionsProvider = ({ children }) => {
  ...
  const [currentCode, setCurrentCode] = useState(selectedQuestion ? selectedQuestion.codeTemplate.functionSignature : ""); // Store current code
  ...
  async function generateCppWithTests(userCode, testCases, codeTemplate) {
    ...
    testCases.forEach((tc, index) => {
      ...
      int result_${index} = ${functionName}(arr1_${index}, ${tc.size1}, arr2_${index}, ${tc.size2},  ${param});
      cout << "Test ${index + 1}: " << (result_${index} == ${tc.expected} ? "✅ Pass" : "❌ Fail") << endl;
      `;
    });
  }
    
  if(codeTemplate.oneArray){  // for One array and one number
    testCases.forEach((tc, index) => {
      const param = tc[codeTemplate.lastparam];
      const arr1Str = tc.arr1.join(',');
      const arr2Str = tc.arr2.join(',');
      ...
      testCode += `
      int arr1_${index}[] = {${arr1Str}};
      int result_${index} = ${functionName}(arr1_${index}, ${tc.size1},  ${param});
      cout << "Test ${index + 1}: " << (result_${index} == ${tc.expected} ? "✅ Pass" : "❌ Fail") << endl;
      `;
    });
  }
